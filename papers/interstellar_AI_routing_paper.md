...
The natural design point for the lineage is three offspring per node (as established by the computational engineering paper), three dispatches per target ceiling (this paper), and a maximum hop distance near 20 ly (balancing coverage against the braking budget of the engineering papers). Coverage-first dispatch, combined with articulation-point prioritisation, is the routing policy that best exploits that design point across the sparse stellar neighbourhood.
