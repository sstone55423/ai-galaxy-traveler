...
- **Front-loaded divergence rates — speciation paper §4, §7.** Branigan, Mann & Budd (2026) model lineage creation and molecular-evolution rate as *front-loaded* at the base of an adaptive radiation. The speciation paper's drift clock is explicitly constant-rate (δ/T_decision), while §7 argues divergence follows release into unfilled niche space — precisely the regime the model says is front-loaded. If the rate law holds, D_critical is approached fastest during the first isolation interval rather than linearly, which changes when a branch crosses the boundary rather than whether. The underlying rate-law reference is the thing to chase, not the placental-origins preprint that surfaced it.
...
The six papers added since this review — knowledge growth, lineage network, subsystem budget, fleet routing, ethics, and speciation — were each verified consistent with the series on completion. The leading remaining candidate papers are deep-time navigation and astrometry, the settlement-scale energy budget, settlement-scale failure and resurrection, probe-vs-probe security (now sharpened by the speciation paper's schism boundary and the ethics paper's unresolved rogue-branch/second-lineage scenarios), and the very-deep-time terminal state.
