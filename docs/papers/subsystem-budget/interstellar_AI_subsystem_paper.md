...
The hardware class is flight-qualified at the relevant scale. The DESTINY+ μ10 microwave-discharge ion engine system delivers 40 mN from four thrusters for under 2 kW at a system mass of ~80 kg, with specific impulse of 3,000–3,300 s and more than 17,000 hours of demonstrated neutralizer endurance (Tabata et al. 2026). The module carried here is a single-string derivative of that class at a fraction of the mass and power; what no current programme qualifies is the millennial dormancy-and-restart cycle, which remains the open engineering item for this line.
...
The 100 kg allocation is generous against contemporary space-compute mass scaling, and the gap is informative. A lifecycle model for megawatt-class orbital data centres puts deployed mass at ~50–75 kg per kW of compute, inclusive of supporting power and thermal hardware (Turyshev 2026); at the standard profile's 50 W of cognitive draw that scaling would give a few kilograms. The seed's 100 kg is therefore not a throughput budget but a survival budget — radiation hardening, k-of-n redundancy, and secondary shielding for a transit measured in millennia — and the difference between the two figures is the price of doing without the servicing and spare-inventory logistics, roughly 5–9 t per MW-year in that model, that an orbital installation assumes and a probe cannot have.
...
Tabata, K., Tsukizaki, R., Imai, S., Morishita, T., Yamashita, Y., & Nishiyama, K. (2026). Development and testing of the DESTINY+ microwave discharge ion engine system. *Journal of Electric Propulsion*. doi:10.1007/s44205-026-00219-4
